...
rtype_re  = re.compile(r'''^(?P<instr>(or|and|add|sub|sllv|srlv|srav|slt|addp8|subp8|addh8|subh8))\s+(?P<rd>\$r[0,1,2,3])\s+(?P<rs>\$r[0,1,2,3])\s+(?P<rt>\$r[0,1,2,3])$''')
immed_re  = re.compile(r'''^(?P<instr>(ori|addi|andi|shuff))\s+(?P<rt>\$r[0,1,2,3])\s+(?P<rs>\$r[0,1,2,3])\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)$''')
twoReg_re = re.compile(r'''^(?P<instr>(ori|addi|andi|shuff))\s+(?P<rt>\$r[0,1,2,3])\s+(?P<rs>\$r[0,1,2,3])$''')
...
signed_re = re.compile(r'addi|beq|bne|lw|sw|setreg|shuff')
both_allowed_re = re.compile(r'ori|andi')
#noargs_re = re.compile(r'''^(?P<instr>(clearreg))$''')
setreg_re   = re.compile(r'''^(?P<instr>setreg)\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)$''')
...
def hexToSigned( immediate, lineNo):
    if (immediate < 2 ** 7  and immediate >=0 ):
        return immediate
    if (immediate < 2 ** 8  and immediate >= (2 ** 7)):
        return immediate-(2**8)
    raise AssemblerSyntaxError(lineNo,"signed hex immediate out of range")   

def toImmediate( signed, immediate_str, lineNo):
    if( signed and immediate_str.startswith("0x")): #if hex digit
        return hexToSigned( int(immediate_str,0), lineNo)
    return int(immediate_str,0)
...
def assemble_instructions(inputFile):
  ...
  for line in inputFile:
    ...
    #noargs   = noargs_re.match(instruction)
    setreg = setreg_re.match(instruction)
    ...
    if len(instruction) != 0:
      ...
      if rtype:
        ...
        immediate = toImmediate(signed, immed.group('immed'), lineNo)
        ...
        opcode = opcodes[setreg.group('instr')]
        immediate = toImmediate( signed, setreg.group('immed'), lineNo)
        imm_check( signed,both_allowed,immediate,lineNo)
        num = opcode << 12 | (immediate & 255)
        debug("instruction: %s num: %04x" % (instruction,num))
      else:

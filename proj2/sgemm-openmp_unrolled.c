...
#pragma omp parallel
  {
   ...
   __m128 c0,c1,c2,c3, c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16;
   __m128 cc0,cc1,cc2,cc3,cc4,cc5,cc6,cc7,cc8,cc9,cc10,cc11,cc12,cc13\
    , cc14,cc15,cc16;
   __m128 b1, b2, b3, b4;
 float tempC[4];
  int ii=0;
...
#pragma omp for
           for(int l=0;l<JB;l++){
for( int j = 0; j < Blockj; j+=4 ) {
          int jj=j+l*Blockj;
             jjma=jj*m_a;
             jjna=jj*n_a;
for (int w=0;w<IB;w++){

    for( int i = 0; i <Blocki/* m_a*/; i+=8 ) {
        int ii=i+w*Blocki;
      //ii=i;  
      iina=ii*n_a;
 ...
 a5 = _mm_loadu_ps(tempA+4*n_a);
 a6 = _mm_loadu_ps(tempA+5*n_a);
 a7 = _mm_loadu_ps(tempA+6*n_a);
 a8 = _mm_loadu_ps(tempA+7*n_a);   
 ///////////////////////////////////////
 ...
 cc1=_mm_add_ps(cc1,  _mm_mul_ps(a5, b1));
 cc2=_mm_add_ps(cc2,  _mm_mul_ps(a6, b1));
 cc3=_mm_add_ps(cc3,  _mm_mul_ps(a7, b1));
 cc4=_mm_add_ps(cc4,  _mm_mul_ps(a8, b1));  
 //
 ...
 cc5=_mm_add_ps(cc5,  _mm_mul_ps(a5, b2));
 cc6=_mm_add_ps(cc6,  _mm_mul_ps(a6, b2));
 cc7=_mm_add_ps(cc7,  _mm_mul_ps(a7, b2));
 cc8=_mm_add_ps(cc8,  _mm_mul_ps(a8, b2));
 //
 ...
 cc9=_mm_add_ps(cc9,  _mm_mul_ps(a5, b3));
 cc10=_mm_add_ps(cc10,  _mm_mul_ps(a6, b3));
 cc11=_mm_add_ps(cc11,  _mm_mul_ps(a7, b3));
 cc12=_mm_add_ps(cc12,  _mm_mul_ps(a8, b3));
 //
 ...
 cc13=_mm_add_ps(cc13,  _mm_mul_ps(a5, b4));
 cc14=_mm_add_ps(cc14,  _mm_mul_ps(a6, b4));
 cc15=_mm_add_ps(cc15,  _mm_mul_ps(a7, b4));
 cc16=_mm_add_ps(cc16,  _mm_mul_ps(a8, b4));
 /////////////////////////////////////////
   ...
   * */
   ...
   c0=  _mm_hadd_ps(cc1,cc1);
    c0=  _mm_hadd_ps(c0,c0); 
 
    tempC[0]=_mm_cvtss_f32(c0);
    c0=  _mm_hadd_ps(cc2,cc2);
    c0=  _mm_hadd_ps(c0,c0);
    tempC[1]= _mm_cvtss_f32(c0); 
     
    c0=  _mm_hadd_ps(cc3,cc3);
    c0=  _mm_hadd_ps(c0,c0);   
    
    tempC[2]=_mm_cvtss_f32(c0);

    c0=  _mm_hadd_ps(cc4,cc4);
    c0=  _mm_hadd_ps(c0,c0);   
  ...
  C[ii+7+jjma]+=tempC[3]; 
  ////////////////////////////////
   c0=  _mm_hadd_ps(cc5,cc5);
    c0=  _mm_hadd_ps(c0,c0); 
 
    tempC[0]=_mm_cvtss_f32(c0);
    c0=  _mm_hadd_ps(cc6,cc6);
    c0=  _mm_hadd_ps(c0,c0);
    tempC[1]= _mm_cvtss_f32(c0); 
     
    c0=  _mm_hadd_ps(cc7,cc7);
    c0=  _mm_hadd_ps(c0,c0);   
    
    tempC[2]=_mm_cvtss_f32(c0);

    c0=  _mm_hadd_ps(cc8,cc8);
    c0=  _mm_hadd_ps(c0,c0);   
    
    tempC[3]= _mm_cvtss_f32(c0);  
  C[ii+4+jjma+m_a]+=tempC[0];
  C[ii+5+jjma+m_a]+=tempC[1];
  C[ii+6+jjma+m_a]+=tempC[2];
  C[ii+7+jjma+m_a]+=tempC[3];
 
  ////////////////////////////////
   c0=  _mm_hadd_ps(cc9,cc9);
    c0=  _mm_hadd_ps(c0,c0); 
 
    tempC[0]=_mm_cvtss_f32(c0);
    c0=  _mm_hadd_ps(cc10,cc10);
    c0=  _mm_hadd_ps(c0,c0);
    tempC[1]= _mm_cvtss_f32(c0); 
     
    c0=  _mm_hadd_ps(cc11,cc11);
    c0=  _mm_hadd_ps(c0,c0);   
    
    tempC[2]=_mm_cvtss_f32(c0);

    c0=  _mm_hadd_ps(cc12,cc12);
    c0=  _mm_hadd_ps(c0,c0);   
    
    tempC[3]= _mm_cvtss_f32(c0);  
  C[ii+4+jjma+2*m_a]+=tempC[0];
  C[ii+5+jjma+2*m_a]+=tempC[1];
  C[ii+6+jjma+2*m_a]+=tempC[2];
  C[ii+7+jjma+2*m_a]+=tempC[3];
   ////////////////////////////////
   c0=  _mm_hadd_ps(cc13,cc13);
    c0=  _mm_hadd_ps(c0,c0); 
 
    tempC[0]=_mm_cvtss_f32(c0);
    c0=  _mm_hadd_ps(cc14,cc14);
    c0=  _mm_hadd_ps(c0,c0);
    tempC[1]= _mm_cvtss_f32(c0); 
     
    c0=  _mm_hadd_ps(cc15,cc15);
    c0=  _mm_hadd_ps(c0,c0);   
    
    tempC[2]=_mm_cvtss_f32(c0);

    c0=  _mm_hadd_ps(cc16,cc16);
    c0=  _mm_hadd_ps(c0,c0);   
    
    tempC[3]= _mm_cvtss_f32(c0);  
  C[ii+4+jjma+3*m_a]+=tempC[0];
  C[ii+5+jjma+3*m_a]+=tempC[1];
  C[ii+6+jjma+3*m_a]+=tempC[2];
  C[ii+7+jjma+3*m_a]+=tempC[3];    

...
*/
/*second version of add with alternating element
 void v_add(double* x, double* y, double* z) {
    #pragma omp parallel 
    {   int NumThread= omp_get_num_threads();
        ...
        for(int i=CurrThread;\
            i<ARRAY_SIZE; i=i+Stride) {
            z[i] = x[i] + y[i];
        }
    }
}
*/
/*
 add with continuous chunk of elements
...
void v_add(double* x, double* y, double* z) {
    ...
    {   int NumThread= omp_get_num_threads();
        ...
        for(int i=CurrThread*Stride;\
            i<(CurrThread+1)*Stride; i++) {
            z[i] = x[i] + y[i];
        }
